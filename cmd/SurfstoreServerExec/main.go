...
func main() {
	...
	// Use tail arguments to hold BlockStore addresses
	args := flag.Args()
	// We want an array of blockStore addresses
...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	g_s := grpc.NewServer()
	if serviceType == "both" {
		// Register the MetaStore and BlockStore services
		MetaStore := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(g_s, MetaStore)

		BlockStore := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(g_s, BlockStore)

	} else if serviceType == "meta" {
		// Register the MetaStore service
		MetaStore := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(g_s, MetaStore)
	} else if serviceType == "block" {
		// Register the BlockStore service
		BlockStore := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(g_s, BlockStore)
	}
